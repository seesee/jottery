...
pub fn load_archived_notes(app: &mut App) -> Result<()> {
    if let (Some(db), Some(key)) = (&app.db, &app.key) {
        let repo = NoteRepository::new(db.connection());
        app.notes = repo.get_archived(key)?;
    }
    Ok(())
}
...
pub fn unarchive_note(app: &mut App) -> Result<()> {
    if let (Some(db), Some(key)) = (&app.db, &app.key) {
        if !app.notes.is_empty() && app.selected_note < app.notes.len() {
            let note_id = app.notes[app.selected_note].id.clone();

            // Unarchive the note by setting archived = false
            if let Some(note) = app.notes.iter_mut().find(|n| n.id == note_id) {
                note.unarchive();
            ...
            // Reload archived notes to refresh the list
            load_archived_notes(app)?;

            // Adjust selection after unarchive
            if app.selected_note >= app.notes.len() && app.selected_note > 0 {
                app.selected_note -= 1;
            }
        }
